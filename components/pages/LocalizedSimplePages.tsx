...
export function LocalizedAboutPageContent() {
  ...
  return (
    <main id="main-content" className="section-shell bg-[#fafbfc]">
      <div className={layoutGutterXClass}><div className={layoutContentMaxClass}><div className="frost-card rounded-2xl p-5 sm:p-7 md:p-10">
        ...
        <div className="relative mt-8 aspect-[16/9] w-full overflow-hidden rounded-xl border border-[#2a3f54]/10 shadow-[0_10px_30px_rgba(0,0,0,0.08)] sm:mt-12 sm:rounded-2xl">
          <Image src={chiSiamoPageImage.src} alt={isEn ? "Studio Capoferri team during a technical meeting in Adro" : chiSiamoPageImage.alt} fill className="object-cover" sizes="(min-width:800px) 800px, 100vw" />
        </div>
      </div></div></div>
    </main>
  );
...
export function LocalizedServicesPageContent() {
  ...
  return (
    <main id="main-content" className="section-shell bg-[#fafbfc]">
      <div className={layoutGutterXClass}><div className={layoutContentMaxClass}><article className="frost-card rounded-2xl p-5 sm:p-7 md:p-10">
        <h1 className={`${fontDisplay.className} ${ui.pageTitle} mb-6 sm:mb-8`}>{isEn ? "Services" : "Servizi"}</h1>
        <p className={ui.body}>{isEn ? "Studio Capoferri provides integrated engineering, architecture and technical consultancy services across every project phase." : "Studio Capoferri offre una gamma completa di servizi di ingegneria, architettura e consulenza tecnica a 360 gradi."}</p>
        <h2 id="progettazione-strutturale" className={sectionHeading}>{isEn ? "Structural design" : "Progettazione strutturale"}</h2>
        <p className={ui.bodyMuted}>{isEn ? "Steel, reinforced concrete and masonry structures, FEM analysis, seismic checks and fire design." : "Strutture in acciaio, calcestruzzo e muratura, analisi FEM, verifiche sismiche e progettazione antincendio."}</p>
        <h2 id="urbanistica-architettura" className={sectionHeading}>{isEn ? "Planning and architecture" : "Urbanistica e architettura"}</h2>
        <p className={ui.bodyMuted}>{isEn ? "Architectural design, implementation plans, planning amendments and compliance analysis." : "Progettazione architettonica, piani attuativi, varianti urbanistiche e analisi di conformità."}</p>
        ...
        <p className={ui.bodyMuted}>{isEn ? "Technical supervision, site assistance, assessments on existing buildings and technical reports." : "Supervisione tecnica, assistenza in cantiere, verifiche su edifici esistenti e relazioni tecniche."}</p>
        <h2 id="servizi-tecnici" className={sectionHeading}>{isEn ? "Technical and cadastral services" : "Servizi tecnici e catastali"}</h2>
        <p className={ui.bodyMuted}>{isEn ? "Building permits, cadastral services, heritage authority procedures, administrative testing and energy certificates." : "Pratiche edilizie, catastali, pratiche per la Sovrintendenza, collaudi amministrativi e certificazioni energetiche."}</p>
        <h2 id="sicurezza-cantieri" className={sectionHeading}>{isEn ? "Site safety" : "Sicurezza cantieri"}</h2>
        <p className={ui.bodyMuted}>{isEn ? "Safety coordination during design and execution, with safety plans and risk management." : "Coordinamento della sicurezza in fase di progettazione ed esecuzione, piani di sicurezza e gestione rischi."}</p>
        <h2 id="assistenza-immobiliare" className={sectionHeading}>{isEn ? "Property support" : "Assistenza immobiliare"}</h2>
        <p className={ui.bodyMuted}>{isEn ? "Technical support for property transactions, appraisals, valuations and condition assessments." : "Supporto tecnico per compravendite, perizie, stime immobiliari e valutazione dello stato di fatto."}</p>
        <ContactCtaSection title={isEn ? "Need technical support for your project?" : "Cerchi supporto tecnico per il tuo progetto?"} description={isEn ? "Contact us for an initial assessment: we can outline timing, approvals and the most suitable design path." : "Contattaci per una valutazione preliminare: ti indichiamo tempi, iter autorizzativi e il percorso progettuale più adatto."} className="mt-14" />
      </article></div></div>
    </main>
  );
}
...
export function LocalizedContactsPageContent() {
  ...
  return (
    <main id="main-content" className="section-shell bg-[#fafbfc]">
      <div className={layoutGutterXClass}><div className={layoutContentMaxClass}>
        <div className="mb-6 max-w-[780px] sm:mb-8"><h1 className={`${fontDisplay.className} ${ui.pageTitle} mb-3 sm:mb-4`}>{isEn ? "Contact" : "Contatti"}</h1><p className={ui.body}>{isEn ? "We are available for preliminary assessments, quotations and technical support on structural design, site supervision and specialist consultancy." : "Siamo disponibili per valutazioni preliminari, preventivi e supporto tecnico su progettazione strutturale in acciaio, direzione lavori e consulenza specialistica."}</p></div>
        <div className="grid gap-5 sm:gap-8 lg:grid-cols-[1.02fr_1.28fr] lg:items-stretch">
          <section aria-labelledby="recapiti-heading" className="frost-card rounded-2xl p-5 sm:p-7 md:p-8">
            ...
            <p className={`mb-4 font-semibold text-[#2a2a2a] sm:mb-6 ${ui.body}`}>{site.name} - {site.tagline}</p>
            <ul className={`space-y-2 sm:space-y-3 ${ui.body}`}>
              <li><strong>{isEn ? "Address" : "Indirizzo"}:</strong> {site.addressLine}</li>
              <li><strong>{isEn ? "Phone" : "Telefono"}:</strong> <a href={`tel:${site.phoneTel}`} title={linkTitles.telefono(site.phoneDisplay)} className="inline-block min-h-[44px] py-1 text-[#333] underline-offset-2 hover:text-[#2a3f54] hover:underline">{site.phoneDisplay}</a></li>
              <li><strong>Email:</strong> <a href={`mailto:${site.email}`} title={linkTitles.email(site.email)} className="inline-block min-h-[44px] py-1 text-[#333] underline-offset-2 hover:text-[#2a3f54] hover:underline">{site.email}</a></li>
              <li><strong>{isEn ? "Hours" : "Orari"}:</strong> {site.openingHoursDisplay}</li>
            </ul>
            <div className="mt-5 border-t border-[#2a3f54]/10 pt-4 sm:mt-7 sm:pt-6"><a href={site.linkedin} target="_blank" rel="noopener noreferrer" title={linkTitles.linkedin} aria-label={isEn ? "Follow us on LinkedIn - Studio Capoferri" : "Seguici su LinkedIn - Studio Capoferri"} className="group focus-ring inline-flex min-h-[44px] items-center gap-3 py-1 text-sm font-semibold text-[#2a3f54] transition hover:text-[#0A66C2]"><span className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[#2a3f54]/15 bg-[#2a3f54]/5 text-[#2a3f54] transition group-hover:border-[#0A66C2] group-hover:bg-[#0A66C2] group-hover:text-white"><LinkedInIcon className="h-4 w-4" /></span>{isEn ? "Follow us on LinkedIn" : "Seguici su LinkedIn"}</a></div>
          </section>
          <section aria-labelledby="mappa-heading" className="frost-card rounded-2xl p-3 sm:p-4 md:p-5"><h2 id="mappa-heading" className={`${fontDisplay.className} ${ui.cardHeading} mb-3 px-1 sm:mb-4 sm:px-2`}>{isEn ? "Where we are" : "Dove siamo"}</h2><MapEmbed /></section>
        </div>
        <section id="form-contatti" className={`mt-10 sm:mt-16 ${scrollAnchorClass}`}><div className="frost-card rounded-2xl p-5 sm:p-7 md:p-8"><h2 className={`${fontDisplay.className} ${ui.cardHeading} mb-2 sm:mb-3`}>{isEn ? "Contact us" : "Contattaci"}</h2><p className={`copy-rhythm mb-6 w-full sm:mb-8 ${ui.bodyMuted}`}>{isEn ? "Fill in the form with the details of your project. Our team will reply with a focused technical response." : "Compila il form con i dettagli del tuo intervento. Riceverai un riscontro tecnico puntuale dal nostro team."}</p><ContactForm /></div></section>
      </div></div>
    </main>
  );
}
    ...
    <main id="main-content" className="section-shell bg-[#fafbfc]">
      <div className={layoutGutterXClass}><div className={layoutContentMaxClass}>
        ...
        <div className="lazy-section grid gap-6 sm:gap-10 md:grid-cols-3">
          {projectCards[locale].map(([title, caption, image, alt, href]) => <div key={href} className="reveal-block"><ProjectPreviewCard href={localizeHref(href, locale)} title={title} caption={caption} image={image} alt={alt} /></div>)}
        </div>
        <ContactCtaSection title={isEn ? "Would you like to develop a project with us?" : "Vuoi realizzare un progetto con noi?"} description={isEn ? "From feasibility to construction: tell us your objectives, timing and constraints." : "Dalla fattibilità al cantiere: raccontaci obiettivi, tempi e vincoli del tuo intervento."} />
      </div></div>
    </main>
  );
}
...
export function LocalizedPrivacyPageContent() {
  ...
  return (
    <main id="main-content" className="section-shell bg-[#fafbfc]">
      <div className={layoutGutterXClass}><div className={layoutContentMaxClass}><article className="max-w-[860px]">
        <h1 className={`${fontDisplay.className} ${ui.pageTitle} mb-7 sm:mb-10`}>Privacy policy</h1>
        <p className={`mb-7 sm:mb-10 ${ui.body}`}><strong>{isEn ? "Data controller" : "Titolare del trattamento"}:</strong> Studio Capoferri SRL STP – Via Piave 35, Adro (BS) – VAT 04732710985 – Email: <a href={`mailto:${site.email}`} title={linkTitles.email(site.email)} className="text-[#2a3f54] underline">{site.email}</a></p>
